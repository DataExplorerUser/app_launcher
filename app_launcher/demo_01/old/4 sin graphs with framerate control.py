...
def update_graphs(sender, data):
    ...
    if frame_count >= rate:
        ...
        y_coordinates.append(sin(x)*10)
        graphs = ['Graph 1', 'Graph 2', 'Graph 3', 'Graph 4']
        for graph in graphs:
            dpg.clear_plot(graph) 
            dpg.add_line_series(graph, "", x_coordinates, y_coordinates)
...
def initiate_graphs():
    ...
    dpg.add_drag_float("RateChange", label="Update Rate", default_value=1, 
        min_value=0.016, max_value=10, speed=0.01, width=600) 
    dpg.add_spacing(count=10)
...
def primary_window():
    with sdpg.window("Primary Window"):
    ...
    dpg.start_dearpygui(primary_window="Primary Window")
